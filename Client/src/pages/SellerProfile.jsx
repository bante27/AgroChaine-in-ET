...
const SellerProfile = () => {
    ...
    const fetchSellerData = async () => {
      ...
      try {
        ...
        if (resSeller.data.success) {
          const user = resSeller.data.user;
          setSeller(user);
          setPostedProducts(user.postedProducts || []);
        } else {
  ...
  if (loading)
    return <p className="text-center mt-10 text-gray-500">Loading seller info...</p>;
  if (error) return <p className="text-center mt-10 text-red-600">{error}</p>;
  if (!seller)
    return <p className="text-center mt-10 text-gray-700">No seller info available.</p>;

  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-emerald-100 to-white px-4 py-8">
      <div className="max-w-7xl mx-auto space-y-10">
        {/* Seller Info Card */}
        <div className="flex flex-col md:flex-row items-center gap-6 bg-white p-6 rounded-3xl shadow-xl border border-gray-100 hover:shadow-2xl transition duration-300">
          <img
            src={seller.profilePic ? `http://localhost:5000${seller.profilePic}` : "https://via.placeholder.com/150"}
            alt={seller.fullName}
            className="w-32 h-32 md:w-40 md:h-40 rounded-full object-cover border-4 border-emerald-400 shadow-lg"
          />
          <div className="flex-1">
            <h1 className="text-4xl font-bold text-gray-900 mb-4">{seller.fullName}</h1>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 text-gray-700 text-sm md:text-base">
              ...
              <p className="flex items-center gap-2"><FaCalendarAlt className="text-emerald-500"/> Joined: {new Date(seller.registrationDate).toLocaleDateString()}</p>
            </div>
          </div>
        </div>

        {/* Posted Products Grid */}
        <div className="bg-white p-6 rounded-3xl shadow-xl border border-gray-100 hover:shadow-2xl transition duration-300">
          <h2 className="text-3xl font-semibold mb-6 text-gray-900">Posted Products</h2>
          {postedProducts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {postedProducts.map((prod) => {
                ...
                return (
                  <div key={prod._id} className="bg-white border rounded-2xl shadow-md hover:shadow-xl transition overflow-hidden">
                    <div className="relative group">
                      <img
                        src={prod.images?.[0] ? `http://localhost:5000${prod.images[0]}` : "https://via.placeholder.com/300"}
                        alt={prod.title}
                        className="w-full h-56 object-cover group-hover:scale-105 transition-transform duration-300"
                      />
                      <span
                        className={`absolute top-3 left-3 text-xs px-2 py-1 rounded-full font-semibold shadow-md ${
                          availableCount > 0 ? "bg-emerald-600 text-white" : "bg-red-600 text-white"
                        }`}
                      >
                    ...
                    <div className="p-4 flex flex-col gap-2">
                      ...
                      <p className="text-gray-700 text-sm">{prod.price ? `${prod.price} ETB` : "Price N/A"}</p>
                      ...
                      {prod.reviews?.length > 0 && (
                        <div className="mt-3 border-t pt-2">
                          {reviewsToShow.map((rev, i) => (
                            <div key={i} className="mb-1 bg-gray-50 p-2 rounded text-xs text-gray-500">
